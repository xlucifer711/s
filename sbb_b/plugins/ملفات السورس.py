...
from ..helpers.utils import _sbb_butils, parse_pre, yaml_format
...
@sbb_b.ar_cmd(
    ...
    info={
        "header": "To list all plugins in sbb_b.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    ...
    o = (await _sbb_butils.runcmd(cmd))[0]
...
async def _(event):
    "To show all config values in sbb_b"
    cmd = "env"
    o = (await _sbb_butils.runcmd(cmd))[0]
    OUTPUT = (
