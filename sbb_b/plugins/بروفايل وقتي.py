...
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, sbb_b, logging
...
digital_group_pic_path = os.path.join(os.getcwd(), "sbb_b", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "sbb_b", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "sbb_b", "photo_pfp.png")
...
jep = Config.DEFAULT_PIC or "sbb_b/helpers/styles/PaybAck.ttf"
...
@sbb_b.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@sbb_b.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@sbb_b.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@sbb_b.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@sbb_b.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
sbb_b.loop.create_task(digitalgrouppicloop())
sbb_b.loop.create_task(autoname_loop())
sbb_b.loop.create_task(autobio_loop())
sbb_b.loop.create_task(group_loop())
