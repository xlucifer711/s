...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_b"):
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب ببوت كرستين **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/cr_source"),
                Button.url(
                    "المطور",
                    "t.me/wjj_u",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل بوت كرستين من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[ᥴ.𝘳 𝘴ꪮꪊ𝘳ᥴꫀ♥](https://t.me/cr_source)",
