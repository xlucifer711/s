...
async def digitalpicloop():
    DIGITALPICSTART = gvarstatus("digitalpic") == "False"
    i = 0
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "False":
...
@sbb_b.ar_cmd(pattern="الص3 اقبل ٢٢$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
        pass
    if gvarstatus("digitalpic") is not None and gvarstatus("digitalpic") == "False":
...
@sbb_b.ar_cmd(pattern="اسم وقتي2$")
async def _(event):
...
@sbb_b.ar_cmd(pattern="33ايو 22تي2$")
async def _(event):
    if gvarstatus("autobio") is not None and gvarstatus("autobio") == "False":
...
async def _(event):
    ...
    if input_str == "2332ايو تي" or input_str == "البايو الوقتي":
        if gvarstatus("autobio") is not None and gvarstatus("autobio") == "False":
            delgvar("autobio")
            await event.client(
