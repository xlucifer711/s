...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await sbb_b.upload_file(auto_group_photo_path)
        try:
            if i > 0:
                async for photo in sbb_b.iter_profile_photos(int(dgp), limit=1) :
                    await sbb_b(
            ...
            return await sbb_b.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
