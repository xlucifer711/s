...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1050898456:
        return await edit_or_reply(mention, f"**- لكك دي هذه قمر وعلى راسك**")
    if user.id == 1355571767:
...
async def permalink(mention):
    ...
    if user.id == 1355571767:
        return await edit_or_reply(mention, f"**100%**")
    if user.id == 1050898456:
        return await edit_or_reply(mention, f"**انا بنت والمطورة يهبل😂**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
