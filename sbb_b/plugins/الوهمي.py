...
@sbb_b.on(admin_cmd(pattern="للكل تاك$"))
...
@sbb_b.on(admin_cmd(pattern=r"سبونج"))
async def kerz(kerz):
    await kerz.edit(R)
...
@sbb_b.on(admin_cmd(pattern=r"كلب"))
async def dog(dog):
    await dog.edit(M)
Z = (
    "┈┏━╮╭━┓┈╭━━━━╮\n"
...
@sbb_b.on(admin_cmd(pattern=r"ذئب"))
async def fox(fox):
    await fox.edit(H)


@sbb_b.on(admin_cmd(pattern=r"فيل"))
async def elephant(elephant):
    await elephant.edit(A)


@sbb_b.on(admin_cmd(pattern=r"هومر"))
async def homer(homer):
    await homer.edit(N)


@sbb_b.on(admin_cmd(pattern=r"بك"))
async def pig(pig):
    await pig.edit(Z)
